...
class Media {
  ...
  padding!: number; 
  width!: number; 
  widthTotal!: number; x!: number; 
  speed: number = 0; 
  isBefore: boolean = false; 
  isAfter: boolean = false;
  opacity: number = 0;
  opacityTarget: number = 0;
  ...
  constructor({ geometry, gl, image, index, length, renderer, scene, screen, text, viewport, bend, textColor, borderRadius = 0, font }: MediaProps) {
    this.geometry = geometry; this.gl = gl; this.image = image; this.index = index; this.length = length; this.renderer = renderer; this.scene = scene; this.screen = screen; this.text = text; this.viewport = viewport; this.bend = bend; this.textColor = textColor; this.borderRadius = borderRadius; this.font = font;
  ...
  createShader() {
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        uniform float uOpacity; /* YENİ: Opaklık kontrolü */
        varying vec2 vUv;
        ...
        void main() {
          ...
          /* DEĞİŞİKLİK: Rengi uOpacity ile çarpıyoruz */
      ...
      uniforms: {
        ...
        uOpacity: { value: 0 } // Başlangıçta 0 (Görünmez)
      },
      transparent: true
    });
    ...
    img.onload = () => {
      ...
      // GÖRSEL YÜKLENDİ: Artık hedefimiz %100 görünürlük
  ...
  update(scroll: { current: number; last: number }, direction: 'right' | 'left') {
    ...
    if (this.bend === 0) { this.plane.position.y = 0; this.plane.rotation.z = 0; } else {
      const B_abs = Math.abs(this.bend); const R = (H * H + B_abs * B_abs) / (2 * B_abs); const effectiveX = Math.min(Math.abs(x), H);
    ...
    this.opacity = lerp(this.opacity, this.opacityTarget, 0.1);
...
class App {
  ...
  medias: Media[] = []; mediasImages: any[] = []; screen!: { width: number; height: number }; viewport!: { width: number; height: number }; raf: number = 0;
  
  boundOnResize!: () => void; boundOnWheel!: (e: Event) => void; boundOnTouchDown!: (e: MouseEvent | TouchEvent) => void;
  boundOnTouchMove!: (e: MouseEvent | TouchEvent) => void; boundOnTouchUp!: (e: MouseEvent | TouchEvent) => void;

  isDown: boolean = false; startX: number = 0; startY: number = 0;
  config: AppConfig;

  constructor(container: HTMLElement, config: AppConfig) {
    document.documentElement.classList.remove('no-js');
    this.container = container; this.config = config; this.scrollSpeed = config.scrollSpeed || 2;
  ...
  createMedias(items: any[] | undefined, bend: number = 1, textColor: string = '#fff', borderRadius: number = 0, font: string = '') {
    const defaultItems = [{ image: `https://picsum.photos/seed/1/800/600`, text: 'My Projects' }];
  ...
  onTouchUp(e: MouseEvent | TouchEvent) {
    ...
    // TIKLAMA (CLICK) ALGILAYICI MATEMATİĞİ
    ...
    // Eğer fare 5 pikselden az hareket ettiyse bu bir tıklamadır, kaydırma (scroll) değil!
    if (distance < 5 && this.config.onItemClick) {
      const mouseWorldX = ((endX / this.screen.width) - 0.5) * this.viewport.width;
      const mouseWorldY = -((endY / this.screen.height) - 0.5) * this.viewport.height;

      for (const media of this.medias) {
        const halfWidth = media.plane.scale.x / 2;
        const halfHeight = media.plane.scale.y / 2;
        
        if (Math.abs(media.plane.position.x - mouseWorldX) < halfWidth && Math.abs(media.plane.position.y - mouseWorldY) < halfHeight) {
          const originalItems = this.config.items && this.config.items.length ? this.config.items : this.mediasImages;
          const clickedItem = originalItems[media.index % originalItems.length];
          this.config.onItemClick(clickedItem);
          break;
        }
      }
    }
  ...
  onWheel(e: Event) {
    const wheelEvent = e as WheelEvent; const delta = wheelEvent.deltaY || (wheelEvent as any).wheelDelta || (wheelEvent as any).detail;
    this.scroll.target += (delta > 0 ? this.scrollSpeed : -this.scrollSpeed) * 0.2; this.onCheckDebounce();
  }

  onCheck() {
    if (!this.medias || !this.medias[0]) return;
    const width = this.medias[0].width; const itemIndex = Math.round(Math.abs(this.scroll.target) / width);
    const item = width * itemIndex; this.scroll.target = this.scroll.target < 0 ? -item : item;
  }
  ...
  onResize() {
    ...
    const fov = (this.camera.fov * Math.PI) / 180; const height = 2 * Math.tan(fov / 2) * this.camera.position.z; const width = height * this.camera.aspect;
  ...
  update() {
    ...
    this.renderer.render({ scene: this.scene, camera: this.camera }); this.scroll.last = this.scroll.current;
  ...
  addEventListeners() {
    ...
    window.addEventListener('resize', this.boundOnResize); window.addEventListener('mousewheel', this.boundOnWheel); window.addEventListener('wheel', this.boundOnWheel);
    window.addEventListener('mousedown', this.boundOnTouchDown); window.addEventListener('mousemove', this.boundOnTouchMove); window.addEventListener('mouseup', this.boundOnTouchUp);
  ...
  destroy() {
    ...
    window.removeEventListener('resize', this.boundOnResize); window.removeEventListener('mousewheel', this.boundOnWheel); window.removeEventListener('wheel', this.boundOnWheel);
    window.removeEventListener('mousedown', this.boundOnTouchDown); window.removeEventListener('mousemove', this.boundOnTouchMove); window.removeEventListener('mouseup', this.boundOnTouchUp);
    ...
    if (this.renderer && this.renderer.gl && this.renderer.gl.canvas.parentNode) { this.renderer.gl.canvas.parentNode.removeChild(this.renderer.gl.canvas as HTMLCanvasElement); }
  }
}
